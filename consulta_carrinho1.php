		...
		while($produto = mysqli_fetch_array($resultado_carrinho))
			...
			if($resultado_produto)
				...
				while($dados_produto = mysqli_fetch_array($resultado_produto))
	                ...
	                echo '<a class="link-custom" href="remover_carrinho.php?produto_id='.$produto_id.'&pagina='.$pagina.'"><button class="btn btn-default btn-block botao">Remover do carrinho</button></a>';
	                echo '</div>';
	                echo '</div>';
	                echo '</div';
					}
